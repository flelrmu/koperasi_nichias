...
export default function GuestRules() {
  ...
  useEffect(() => {
    const fetchPeraturan = async () => {
      try {
        const res = await axios.get(`${API_URL}/peraturan`);
        if (res.data.success) {
    ...
    const fetchSecretaryContact = async () => {
      try {
        const res = await axios.get(`${API_URL}/user/contact/sekretaris`);
        if (res.data.success) {
  ...
  const handleContactAdmin = () => {
    ...
    // Assuming the number in DB is like 0812... or +62...
    ...
    <div className="space-y-12">
      ...
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <p className="text-lg text-gray-500 leading-relaxed">
          Pelajari seluruh kebijakan simpan pinjam dan hak keanggotaan sebelum bergabung dengan Koperasi Karyawan Nichias Sunijaya.
        </p>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between relative z-10">
                    ...
                    <button 
                      onClick={() => navigate(`/rules/${rule.peraturan_id}`)}
                      className="bg-[#004A9C] text-white p-3 rounded-xl transition-all hover:scale-110 shadow-lg shadow-blue-900/10"
                    >
      ...
      >
        ...
        <div className="flex-1 text-center md:text-left relative z-10">
          ...
          <p className="text-white/80 text-lg font-medium">Tim pengurus kami siap membantu menjelaskan setiap poin kebijakan melalui sesi tanya jawab.</p>
        </div>
        <motion.button
          whileHover={{ 
            scale: 1.05, 
